...
def extraer_informacion_pdf(ruta_pdf):
    ...
    tipo_documento_objetivo = "REMITO"
    if tipo_documento_objetivo not in texto_completo or razon_social_objetivo not in texto_completo or cuit_objetivo not in texto_completo:
...
def buscar_pdfs_con_nombre_similar(directorio, patron="CAC01005448-ORG-RX0001-*.pdf"):
    ...
    """
    archivos_pdf = []
    patrones = patron if isinstance(patron, (list, tuple)) else [patron]
    # Recorre los archivos en el directorio especificado
    for archivo in os.listdir(directorio):
        if any(fnmatch.fnmatch(archivo, p) for p in patrones):
